...
export default function ProjectsPage() {
  ...
  const [projectAssignments, setProjectAssignments] = useState<{[key: string]: any[]}>({})
  ...
  const handleStatusChange = async (id: string, newStatus: string) => {
    const { data, error } = await database.updateProject(id, { status: newStatus })
    if (data) {
      setProjects(projects.map(proj => 
  ...
  const handleAssignEmployees = async (e: React.FormEvent) => {
    ...
    const { data, error } = await database.assignEmployeesToProject(assigningProject.id, selectedEmployees)
    
    if (!error) {
      // Reload assignments for this project
      const { data: assignmentData } = await database.getProjectAssignments(assigningProject.id)
      if (assignmentData) {
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => setNewProject({...newProject, hourly_rate: e.target.value ? parseFloat(e.target.value) : 0})}
                    />
                  </div>
                </div>
                <div className="space-y-2">
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => setEditingProject({...editingProject, hourly_rate: e.target.value ? parseFloat(e.target.value) : 0})}
                    />
                  </div>
                </div>
                <div className="space-y-2">
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid md:grid-cols-2 gap-2 max-h-48 overflow-y-auto border rounded-md p-4">
                    {availableEmployees.map(employee => (
                      <div key={employee.id} className="flex items-center space-x-2 hover:bg-gray-50 p-2 rounded cursor-pointer">
                        <input
                          type="checkbox"
                          id={`emp-${employee.id}`}
                        ...
                        />
                        <Label htmlFor={`emp-${employee.id}`} className="text-sm font-normal cursor-pointer flex-1">
                        ...
                        <div className="max-w-xs">
                          {projectAssignments[project.id]?.length ? (
                            <div className="flex flex-wrap gap-1">
                              {projectAssignments[project.id].slice(0, 2).map((assignment) => (
                                <span key={assignment.id} className="px-2 py-1 bg-blue-100 text-blue-800 text-xs rounded">
                                  {assignment.employees?.name}
                                </span>
                              ))}
                              {projectAssignments[project.id].length > 2 && (
                                <span className="px-2 py-1 bg-gray-100 text-gray-600 text-xs rounded">
                                  +{projectAssignments[project.id].length - 2} more
                                </span>
                              )}
                            </div>
                          ) : (
                            <span className="text-muted-foreground text-sm">No employees assigned</span>
                          )}
                        </div>
                      </TableCell>
                        ...
                        <div className="flex gap-2">
                          ...
                          <Button 
                            ...
                            onClick={() => {
                              ...
                              const currentAssignments = projectAssignments[project.id] || []
